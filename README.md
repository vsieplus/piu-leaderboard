...
| `rank` | The rank or range of ranks to query. To query a range, use the format `rank1-rank2`, where `rank1 < rank2`. Rank(s) must be between 1 and 100. To query different ranges at the same time, use a comma to separate them ( e.g. `rank1,rank2-rank3` ) |
